  ...
  return (
    <header className="w-full bg-gradient-to-r from-rose-50 to-indigo-50 text-gray-900 shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
        ...
        {/* Contact Button */}
        <button className="hidden lg:block bg-rose-500 hover:bg-rose-600 text-white px-6 py-2 rounded-full font-medium transition duration-300 transform hover:scale-105 shadow-md shadow-rose-200">
          CONTACT 
        </button>
      ...
      <div className={`lg:hidden bg-gradient-to-b from-rose-50 to-indigo-50 overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96' : 'max-h-0'}`}>
        <div className="pb-4 px-4">
          <nav className="flex flex-col space-y-4 font-medium tracking-wide">
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>HOME</NavLink>
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>ABOUT ME</NavLink>
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>SERVICES</NavLink>
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>PORTFOLIO</NavLink>
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>NEWS</NavLink>
            <NavLink className="hover:text-rose-600 transition py-2 border-b border-rose-100" onClick={() => setIsMenuOpen(false)}>CONTACT</NavLink>
          </nav>
